...
describe('sendRCONCommandToServer', () => {
  ...
  it('dev status and parser', () => {
    return misrcon.sendRCONCommandToServer(dev).then((res) => {
      expect(misrcon.parseStatusResponseToJs(res).name).to.equal('Dev Server');
    }).catch((e) => {
      throw e;
    });
  });
...
describe('getAllServerData', () => {
  ...
  it('us77 all data', (done) => {
    misrcon.getAllServerData(us77).then((res) => {
  ...
  it('br all data', function(done) {
    ...
    misrcon.getAllServerData(br1).then((res) => {
      expect(typeof res).to.equal('object');
      done();
    }).catch((e) => {
      throw e;
