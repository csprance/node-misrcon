...
export function createChallengeResponseString(
	upTime: string,
	password: string
): string {
...
export function parseCommandResponse(str: string): any {
	// server response looks like
...
export function parseAuthResponse(data: string, reject: any): string {
...
export function isIllegalCommand(res: $AxiosXHR<*>): boolean {
	// server response looks like
	...
	parseString(res.data, (err, result) => {
		if (!err) {
			responseString =
				result.methodResponse.params[0].param[0].value[0].string[0];
		}
	});
